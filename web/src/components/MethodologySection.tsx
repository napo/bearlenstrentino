...
// AGENTS.md / REFERENCES.md). Kept as prose here, self-contained in the
// deployed site, rather than only linking out to REFERENCES.md — a
// static export of the site may not bundle the repository's other
// markdown files. "Bias" is kept in parentheses next to a plain-language
// name because it's the term the cited papers themselves use.
...
export function MethodologySection() {
  return (
    <div className="methodology-catalog">
      {BIASES.map((bias) => (
        <article key={bias.title} id={bias.id}>
          <h3>{bias.title}</h3>
          <p>{bias.body}</p>
